...
const APIScrollTelling = () => {
  ...
  return (
    <div style={{ width: '100%', padding: '60px', background: '#FFFFFF', borderRadius: '24px', border: '1px solid #E2E8F0', display: 'flex', gap: '60px', alignItems: 'center', flexWrap: 'wrap' }}>
      {/* Left: Interactive Map */}
      <div style={{ flex: 1, minWidth: '350px', position: 'relative', height: '500px' }}>
        <div style={{ display: 'flex', flexDirection: 'column-reverse', gap: '30px', height: '100%' }}>
            ...
            return (
              <div key={step.id} style={{ flex: 1, position: 'relative' }}>
                <motion.div
                  ...
                  style={{ 
                    background: '#FFFFFF', padding: '24px', borderRadius: '16px', border: '2px solid #E2E8F0', 
                    cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '20px', zIndex: 2, position: 'relative'
                  }}
                >
                  <div style={{ 
                    width: '50px', height: '50px', borderRadius: '12px', background: isActive ? step.color : '#F1F5F9',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.3s'
                  }}>
                    <Icon color={isActive ? '#FFFFFF' : '#94A3B8'} size={24} />
                  </div>
                  <div>
                    <div style={{ fontSize: '0.8rem', fontWeight: 700, color: isActive ? step.color : '#64748B', marginBottom: '2px' }}>STEP 0{index + 1}</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#1E293B' }}>{step.title}</div>
                  </div>
                  {isActive && (
                    <motion.div layoutId="arrow" style={{ marginLeft: 'auto' }}>
                      <ChevronRight color={step.color} />
                    </motion.div>
                  )}
                </motion.div>
                ...
                {index < steps.length - 1 && (
                  <div style={{ position: 'absolute', top: '-30px', left: '49px', width: '2px', height: '30px', background: '#E2E8F0' }}>
                    <motion.div 
      ...
      <div style={{ flex: 1, minWidth: '350px' }}>
        <AnimatePresence mode="wait">
          ...
          >
            <div style={{ color: steps[activeStep].color, fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.1em', marginBottom: '16px' }}>
              {steps[activeStep].id.toUpperCase()} LAYER
            </div>
            <h3 style={{ fontSize: '2.5rem', fontWeight: 900, color: '#0F172A', marginBottom: '24px', lineHeight: 1.2 }}>
            ...
            <div style={{ marginTop: '40px', height: '120px', background: '#F1F5F9', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px dashed #CBD5E1' }}>
              {activeStep === 0 && (
                <div style={{ display: 'flex', gap: '10px' }}>
                  <Laptop size={40} color="#3B82F6" />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <div style={{ width: '80px', height: '8px', background: '#CBD5E1', borderRadius: '4px' }} />
                    <div style={{ width: '60px', height: '8px', background: '#CBD5E1', borderRadius: '4px' }} />
                  </div>
                </div>
              )}
              {activeStep === 1 && (
                <div style={{ position: 'relative' }}>
                  <Server size={40} color="#00D1B2" />
                  <motion.div 
                    animate={{ scale: [1, 1.2, 1], opacity: [0.5, 1, 0.5] }}
                    transition={{ repeat: Infinity, duration: 2 }}
                    style={{ position: 'absolute', inset: -10, border: '2px solid #00D1B2', borderRadius: '50%' }} 
                  />
                </div>
              )}
              {activeStep === 2 && (
                <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
                  {[40, 70, 50, 90].map((h, i) => (
                    <motion.div 
                      ...
                      initial={{ height: 0 }}
                      animate={{ height: h }}
                      style={{ width: '10px', background: '#F59E0B', borderRadius: '5px' }} 
                    />
                  ))}
                  <PieChart size={30} color="#F59E0B" />
                </div>
              )}
            </div>
          </motion.div>
