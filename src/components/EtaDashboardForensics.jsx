...
const EtaDashboardForensics = () => {
  ...
  return (
    <div style={{ width: '100%', minHeight: '500px', background: '#F8FAFC', borderRadius: '24px', padding: '40px', border: '1px solid #E2E8F0', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ color: '#94A3B8', fontSize: '0.75rem' }}>RUNNING</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#EF4444', boxShadow: '0 0 8px #EF4444' }} />
            <span style={{ color: '#94A3B8', fontSize: '0.75rem' }}>FAILED</span>
          </div>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ background: '#1E293B', color: '#94A3B8' }}>
              ...
              <th style={{ padding: '15px 20px', fontWeight: 600 }}>RESULT</th>
            </tr>
          </thead>
          <tbody>
            {testCases.map((tc) => (
              ...
              >
                <td style={{ padding: '18px 20px', fontWeight: 700 }}>{tc.name}</td>
                <td style={{ padding: '18px 20px', color: '#94A3B8' }}>{tc.device}</td>
                ...
                <td style={{ padding: '18px 20px' }}>
                  {tc.result === 'Pass' ? <CheckCircle2 color="#00D1B2" size={18} /> : 
                   (tc.result === 'Failed' ? <AlertCircle color="#EF4444" size={18} /> : '-')}
                </td>
                ...
                <AnimatePresence>
                  {hoveredCase === tc.id && tc.status === 'failed' && (
                    ...
                    >
                      ...
                      <div style={{ width: '100%', height: '150px', background: '#0F172A', borderRadius: '8px', overflow: 'hidden', position: 'relative' }}>
                        {/* Mock Video content */}
      ...
      {/* Floating Info */}
      <div style={{ position: 'absolute', bottom: '30px', left: '40px', color: '#475569', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <Activity size={16} color="#00D1B2" />
